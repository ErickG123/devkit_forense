...
describe_app = typer.Typer(
    help="Ajuda interativa para os comandos/funcionalidades do toolkit forense."
)
...
@describe_app.command(
    "func", help="Mostra a descrição e exemplo de retorno de uma função/command do toolkit."
)
def describe(
    func: str = typer.Argument(
        ..., help="Nome da função que você deseja entender (ex: map, scan, sweep)"
    ),
    full: bool = typer.Option(
        False, "--full", "-f", help="Mostrar exemplo formatado (mais detalhado)."
    ),
):
    key = _find_key(func)
    if not key:
